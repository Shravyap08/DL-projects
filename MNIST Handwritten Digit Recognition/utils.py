...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if seed:
        ...
        np.random.seed(seed)
    if shuffle==True:
        zipped_data = list(zip(data, label))
        random.shuffle(zipped_data)
        data[:], label[:] = zip(*zipped_data)

        #data = [data[i] for i in index]
        #labels = [label[j] for j in label_index]
        
    Total_batches= int((len(data)+batch_size-1)/batch_size)
    
    for i in range(Total_batches):
        data_batches= data[i*batch_size:(i+1)*batch_size]
        #batched_data= np.array(data_batches)
        batched_data.append(np.array(data_batches))
        label_batches= label[i*batch_size:(i+1)*batch_size]
        #batched_data= np.array(data_batches)
        batched_label.append(np.array(label_batches))
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.plot(epochs, valid_acc_history, label='testing accuracy')
