...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        #FORWARD PASS
        Z= np.dot(X,self.weights['W1'])+self.weights['b1']
        A = self.sigmoid(Z)
        C = np.dot(A,self.weights['W2'])+self.weights['b2']
        p = self.softmax(C)
        ...
        dldC=(p-y_o)/N
        dldw2= np.dot(A.T, dldC)
        dldb2= np.sum(dldC, axis=0)
        dldA = np.dot(dldC,self.weights['W2'].T)
        dldZ= dldA*self.sigmoid_dev(Z)
        dldw1= np.dot(X.T, dldZ)
