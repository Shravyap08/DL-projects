...
class CustomReLU(TorchFunc):
    ...
    def backward(self, dout):
        ...
        mask_x_positive = (x > 0).type_as(dout)
        mask_dout_positive = (dout > 0).type_as(dout)
        
        grad= dout*mask_x_positive*mask_dout_positive
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        score_max = torch.sum(scores.gather(1, y_tensor.view(-1, 1)).squeeze())
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        score_max = scores[torch.arange(scores.shape[0]), y_tensor]
        ...
        pooled_gradients_expanded = pooled_gradients[:, :, None, None]
        activations *= pooled_gradients_expanded
